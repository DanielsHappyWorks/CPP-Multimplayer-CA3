...
void Panzer::ProcessInput( float inDeltaTime, const InputState& inInputState )
{
	//process our input....
...
void Panzer::AdjustVelocityByThrust( float inDeltaTime )
{
...
void Panzer::SimulateMovement( float inDeltaTime )
{
	//simulate us...
...
void Panzer::HandleHealthIncrease(int health)
{
		...
		if( target != this && !target->DoesWantToDie() )
		{
			...
			if( distSq < ( collisionDist * collisionDist ) )
			{
				...
				if( target->HandleCollisionWithCat( this ) )
				{
					...
					Panzer* targetCat = target->GetAsCat();
					if( targetCat )
					{
...
void Panzer::ProcessCollisionsWithScreenWalls()
{
...
uint32_t Panzer::Write( OutputMemoryBitStream& inOutputStream, uint32_t inDirtyState ) const
{
